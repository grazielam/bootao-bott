...
class HuTaoBot(commands.Bot):
    ...
    async def setup_hook(self):
        # Registra as Views de forma persistente para que os botões dinâmicos e estáticos continuem funcionando após reiniciar
        self.add_view(ViewAbreTicket())
        self.add_view(ViewPainelAdminGeral())
...
# ==========================================
# 🔘 SISTEMA DE BOTÃO PERSONALIZADO DINÂMICO
# ==========================================
class ViewBotaoDinamicoPersistente(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)

    # Esse receptor captura cliques de qualquer botão gerado dinamicamente pelo painel administrativo
    @discord.ui.button(style=discord.ButtonStyle.primary, custom_id="dinamico_placeholder_id")
    async def botao_clicado(self, interaction: discord.Interaction, button: discord.ui.Button):
        pass # Mantido estrutura padrão para herança de dados interna

    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        # Quando alguém clica no botão customizado, buscamos o que está programado no custom_id dele
        custom_id = interaction.data.get("custom_id", "")
        if custom_id.startswith("msg_custom_"):
            # O texto configurado fica guardado dentro do próprio ID do botão, separado pelo prefixo
            mensagem_para_exibir = custom_id.replace("msg_custom_", "", 1)
            # Entrega a resposta de forma limpa e visível somente para quem clicou!
            await interaction.response.send_message(mensagem_para_exibir, ephemeral=True)
            return True
        return await super().interaction_check(interaction)

# ==========================================
# 📥 MODAIS DO PAINEL DE CONTROLE (ADMINS)
# ==========================================
...
# Passo 2 do Criador de Embed: Configurar o Botão Customizado
class ModalConfigurarBotaoEmbed(discord.ui.Modal, title="🔘 Passo 2: Configurar o Botão"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        # Criamos a View dinamicamente injetando o texto secreto no custom_id do botão
        ...
        view_customizada.add_item(discord.ui.Button(
            ...
            custom_id=id_customizado[:100] # Limite de caracteres nativo do Discord para IDs
        ))

        # Envia a embed estruturada no canal alvo com o botão criado
...
# Passo 1 do Criador de Embed: Estruturar a Embed Principal
class ModalCriarEmbedPersonalizado(discord.ui.Modal, title="🎨 Passo 1: Criar a Embed"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Avança para o Passo 2: Abre o modal de configuração do botão acoplado
        await interaction.response.send_modal(ModalConfigurarBotaoEmbed(canal, embed_construida))


# ==========================================
# 🛠️ PAINEL DE CONTROLE ADMIN INTERATIVO
# ==========================================
class ViewPainelAdminGeral(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)

    @discord.ui.button(label="Enviar", style=discord.ButtonStyle.secondary, custom_id="admin_btn_setup", emoji="▶️")
    ...
    @discord.ui.button(label="Enviar", style=discord.ButtonStyle.secondary, custom_id="admin_btn_embed", emoji="🎨")
    ...
    @discord.ui.button(label="Enviar", style=discord.ButtonStyle.secondary, custom_id="admin_btn_reiniciar", emoji="🔄")
...
async def criar_painel_admin(ctx):
    ...
    # 1. Linha do Setup Painel (Tickets)
    embed_setup = discord.Embed(
        title="**setup_panel**",
        description="Posta o painel de tickets em qualquer canal de texto (admin)",
        color=discord.Color.from_rgb(30, 30, 30)
    )
    view_setup = ViewPainelAdminGeral()
    view_setup.children[1].disabled = True 
    view_setup.children[2].disabled = True 
    await ctx.send(embed=embed_setup, view=view_setup)

    # 2. Linha do Criador de Embeds Personalizados (Com Botão Customizável)
    ...
    view_embed = ViewPainelAdminGeral()
    view_embed.children[0].disabled = True 
    view_embed.children[2].disabled = True 
    await ctx.send(embed=embed_personalizado, view=view_embed)
    ...
    view_reiniciar = ViewPainelAdminGeral()
    view_reiniciar.children[0].disabled = True 
    view_reiniciar.children[1].disabled = True 
    await ctx.send(embed=embed_reiniciar, view=view_reiniciar)
